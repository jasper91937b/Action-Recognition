...
class Tracker(object):
    ...
    def track(self, curr_skels):
        ...
        N = len(curr_skels) # number of humans
        ...
        if len(self._dict_id2skeleton) > 0:
            ...
            # print(f"{ids}") # id = human_id => (id1, id2, id3)
            # print(f"{prev_skels}") # skeletons of human => ([human1], [human2], [human3])
            good_matches = self._match_features(prev_skels, curr_skels)

            self._dict_id2skeleton = {}
            is_matched = [False]*N # [True, False, False]
    ...
    def _sort_skeletons_by_dist_to_center(self, skeletons):
        ...
        # the distance of neck to center
        def cost(skeleton):
        ...
        def cmp(a, b): 
            return (a > b)-(a < b)
        def mycmp(sk1, sk2): return cmp(cost(sk1), cost(sk2)) # if cost(sk1) > cost(2), change
    ...
    def _match_features(self, features1, features2):
        ...
        if n1 and n2:
            ...
            # Find the match of features1[i], find the index
