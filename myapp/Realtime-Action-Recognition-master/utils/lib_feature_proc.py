...
# get_a_normalized_standing_skeleton
# 1. Remove offset by setting neck as origin 把所有x, y點減掉脖子的x, y
# 2. Scale the skeleton by taking neck-thigh distance as height 把所有點除以脖子到腿的距離
...
def extract_multi_frame_features(
    ...
    '''
     ...
     提取身體速度，關節速度和經過標準化的關節位置的特徵。
     參數:  X => 36個骨骼的位置
           images_info = [cnt_action, cnt_clip, cnt_image, action_label, filepath]
           Y => 照片的label images_info[3]
           video_indices => images_info[1]
    '''
    X_new = []
    Y_new = []
    N = len(video_indices) # count how many datas

    # Loop through all data
    for i, _ in enumerate(video_indices):

        # If a new video clip starts, reset the feature generator
        # 如果是新的影片開始，重置FeatureGenerator
...
class FeatureGenerator(object):
    def __init__(self,
        ...
        '''
        self._window_size = window_size # 5
    ...
    def add_cur_skeleton(self, skeleton):
        ...
        x = retrain_only_body_joints(skeleton) # 取出13個骨骼位置，不包含頭部
        ...
        if not ProcFtr.has_neck_and_thigh(x):
            ...
            if len(self._x_deque) < self._window_size:
                ...
                f_poses = self._deque_features_to_1darray(xnorm_list) # 5 images * 26 
                ...
                # -- Output
                features = np.concatenate((f_poses, f_v_joints, f_v_center)) # len = 5*26 + 80 + 104
